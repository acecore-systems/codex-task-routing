...
- 利用者が通常Chat経路を明示的に有効にした場合だけ、ルート親は通常Chatへの送受信を別経路として使える。フックからは起動しない。Work・モデルAPIへの切替は禁止し、UIモデル確認、依頼ID照合、内容の受入を親が行う。
...
- 基準運用は親GPT-6 Sol/xhigh、子GPT-6 Luna/max・Sol/high・Astra/high、適合工程は通常Chat 6 Proを優先する。親の実設定は変更せず、同モデルへの無用な移管をしない。並列は親の独立作業、直列の専門委譲は具体的追加価値とホストの許可を確認し、上位指示を迂回しない。
